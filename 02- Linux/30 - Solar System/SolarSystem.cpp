...
void Render()
{
	...
	glPushMatrix();
		...
		gluSphere(ASM_quadric, 0.75f, 30, 30);

	glPopMatrix();									
										
	glRotatef(ASM_year, 0.0f, 1.0f, 0.0f);
	...
	glPushMatrix();		
		...
		glRotatef(ASM_day, 0.0f, 0.0f, 1.0f);  

		glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);          
		glColor3f(0.4f, 0.9f, 1.0f);						

		ASM_quadric = gluNewQuadric();
		gluSphere(ASM_quadric, 0.2f, 20, 20);
	...
	glPushMatrix();
		...
		glRotatef(ASM_hour, 0.0f, 0.0f, 1.0f);
		...
		ASM_quadric = gluNewQuadric();
		gluSphere(ASM_quadric, 0.1f, 20, 20);
