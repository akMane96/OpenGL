...
void Initialize()
{
	...
	glGenBuffers(1, &ASM_vbo_index);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ASM_vbo_index);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(ASM_squareIndex), ASM_squareIndex, GL_STATIC_DRAW);
...
void UnInitialize()
{
	...
	if (ASM_vbo_index)
	{
		glDeleteBuffers(1, &ASM_vbo_index);
		ASM_vbo_index = 0;
	}
