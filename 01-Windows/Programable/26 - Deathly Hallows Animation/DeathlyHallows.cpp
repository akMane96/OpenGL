...
GLuint ASM_vaoLine;
GLuint ASM_vboPostionLine;
GLuint ASM_vboColorLine;
...
//Calculations
GLfloat ASM_part = sqrtf(3.0f) / 3.0f;
GLfloat ASM_RaidusIncircle = ASM_part;
...
void Initialize()
{
	...
	const GLfloat ASM_lineVerticies[] =
	{
		0.0f, 2 * ASM_part, 0.0f,
		0.0f, -ASM_part, 0.0f
	};
	...
	{
		ASM_XCo[0], ASM_YCo[0], 0.0f,
		ASM_XCo[1], ASM_YCo[1], 0.0f,
		ASM_XCo[1], ASM_YCo[1], 0.0f,
		ASM_XCo[2], ASM_YCo[2], 0.0f,
		ASM_XCo[2], ASM_YCo[2], 0.0f,
		ASM_XCo[0], ASM_YCo[0], 0.0f
	...
	const GLfloat ASM_triangleColor[] =
	{
		...
		//////------------------------------------------------------------ Rectangle
		glGenVertexArrays(1, &ASM_vaoLine);
		glBindVertexArray(ASM_vaoLine);

			glGenBuffers(1, &ASM_vboPostionLine);
			glBindBuffer(GL_ARRAY_BUFFER, ASM_vboPostionLine);
			glBufferData(GL_ARRAY_BUFFER, sizeof(ASM_lineVerticies), ASM_lineVerticies, GL_STATIC_DRAW);
			...
			glGenBuffers(1, &ASM_vboColorLine);
			glBindBuffer(GL_ARRAY_BUFFER, ASM_vboColorLine);
...
void Triangle()
{
	...
	ASM_translateMatrix = translate(-5.0f + tXTriangle, -5.0f + tYTriangle, -8.0f);
	ASM_rotateMatrix = rotate(ASM_RAngleTriangle, 0.0f, 1.0f, 0.0f);
...
void Incircle()
{
	...
	ASM_translateMatrix = translate(5.0f - tXCircle, tYCircle - 5.0f, -8.0f);
	//ASM_translateMatrix = translate(0.0f, 0.0f, -8.0f);
	ASM_rotateMatrix = rotate(ASM_RAngleCircle, 0.0f, 1.0f, 0.0f);
	...
	for (GLfloat ASM_angle = 0.0f; ASM_angle <= (2.0f * M_PI); (ASM_angle = ASM_angle + 0.001f))
	{
		...
		ASM_Point[0] = ASM_RaidusIncircle * sin(ASM_angle);
		ASM_Point[1] = ASM_RaidusIncircle * cos(ASM_angle);
		ASM_Point[2] = 0.0f;
		...
		glBindVertexArray(ASM_vaoPoint);
			...
			glBindBuffer(GL_ARRAY_BUFFER, ASM_vboPostionPoint);
			glBufferData(GL_ARRAY_BUFFER, 3 * sizeof(GLfloat), ASM_Point, GL_DYNAMIC_DRAW);
	...
	/*for (GLfloat ASM_angle = 0.0f; ASM_angle <= (2.0f * M_PI); (ASM_angle = ASM_angle + 0.001f))
	{
		...
		ASM_x = ASM_RaidusIncircle/1000 * sin(ASM_angle);
		ASM_y = ASM_RaidusIncircle/1000 * cos(ASM_angle);
...
void Line()
{
	...
	ASM_translateMatrix = translate(0.0f, sqrtf(50) - tYLine, -8.0f);
	ASM_modelViewMatrix = ASM_translateMatrix;
	ASM_modelViewProjectMatrix = ASM_perspectiveProjectionMatrix * ASM_modelViewMatrix;
...
void UnInitialize()
{
	...
	if (ASM_vaoLine)
	{
		glDeleteVertexArrays(1, &ASM_vaoLine);
		ASM_vaoLine = 0;
	}
	...
	if (ASM_vboPostionLine)
	{
		glDeleteBuffers(1, &ASM_vboPostionLine);
		ASM_vboPostionLine = 0;
	}

	if (ASM_vboColorLine)
	{
		glDeleteBuffers(1, &ASM_vboColorLine);
		ASM_vboColorLine = 0;
	}
