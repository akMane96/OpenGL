...
void Initialize()
{
	...
	glBindVertexArray(ASM_vao);
		...
		glVertexAttrib3f(ASM_ATTRIBUTE_COLOR, 0.898f, 0.721f, 0.529f);
...
void Display()
{
	...
	glUseProgram(ASM_shaderProgramObject);
		...
		ASM_translateMatrix = translate(0.0f, 0.0f, -10.0f);
		...
		ASM_PushMatrix(ASM_modelViewMatrix);
				...
				ASM_rotateMatrix = rotate((GLfloat)ASM_shoulder, 0.0f, 0.0f, 1.0f);

				ASM_modelViewMatrix = ASM_modelViewMatrix * ASM_rotateMatrix * ASM_translateMatrix ;
				ASM_modelViewProjectMatrix = ASM_perspectiveProjectionMatrix * ASM_modelViewMatrix;
				glUniformMatrix4fv(ASM_mvpUniform, 1, GL_FALSE, ASM_modelViewProjectMatrix);
				...
				ASM_PushMatrix(ASM_modelViewMatrix);
					...
					ASM_scaleMatrix = scale(2.0f, 0.5f, 1.0f);
					ASM_modelViewMatrix = ASM_modelViewMatrix * ASM_scaleMatrix;
					ASM_modelViewProjectMatrix = ASM_perspectiveProjectionMatrix * ASM_modelViewMatrix;
					glUniformMatrix4fv(ASM_mvpUniform, 1, GL_FALSE, ASM_modelViewProjectMatrix);
				...
				ASM_translateMatrix = translate(1.0f, 0.0f, 0.0f);
				ASM_rotateMatrix = rotate((GLfloat)ASM_elbow, 0.0f, 0.0f, 1.0f);
				ASM_modelViewMatrix = ASM_modelViewMatrix * ASM_rotateMatrix * ASM_translateMatrix;
				ASM_modelViewProjectMatrix = ASM_perspectiveProjectionMatrix * ASM_modelViewMatrix;
				glUniformMatrix4fv(ASM_mvpUniform, 1, GL_FALSE, ASM_modelViewProjectMatrix);
