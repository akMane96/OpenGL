...
struct ASM_stack_node
{
	mat4 ASM_data;
	struct ASM_stack_node* ASM_next;

};

struct ASM_stack_node* ASM_head = NULL;
...
void Display()
{
	...
	glUseProgram(ASM_shaderProgramObject);
		...
		ASM_scaleMatrix = scale(0.3f, 0.3f, 0.3f);
		...
		ASM_PushMatrix(ASM_modelViewMatrix);

			ASM_rotateMatrix = rotate(0.0f, 1.0f, 0.0f, 0.0f);			
			ASM_modelViewMatrix = ASM_modelViewMatrix * ASM_rotateMatrix;
			...
			glBindVertexArray(ASM_vao);
				...
				glVertexAttrib3f(ASM_ATTRIBUTE_COLOR, 1.0f, 1.0f, 0.0f);
				glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ASM_vbo_sphere_element);
				glDrawElements(GL_TRIANGLES, ASM_gNumElements, GL_UNSIGNED_SHORT, 0);

			glBindVertexArray(0);
		
		ASM_modelViewMatrix = ASM_PopMatrix();
		...
		ASM_PushMatrix(ASM_modelViewMatrix);
			...
			ASM_rotateMatrix = rotate((GLfloat)ASM_year, 0.0f, 1.0f, 0.0f);
			ASM_translateMatrix = translate(1.0f, 0.0f, 0.0f);
			ASM_modelViewMatrix = ASM_modelViewMatrix  * ASM_rotateMatrix * ASM_translateMatrix * ASM_scaleMatrix;

			ASM_modelViewProjectMatrix = ASM_perspectiveProjectionMatrix * ASM_modelViewMatrix;
			glUniformMatrix4fv(ASM_mvpUniform, 1, GL_FALSE, ASM_modelViewProjectMatrix);

			//////------------------------------------------------------- Earth Rotation
			ASM_rotateMatrix = rotate((GLfloat)ASM_day, 0.0f, 1.0f, 0.0f);
			ASM_modelViewMatrix = ASM_modelViewMatrix *  ASM_rotateMatrix;
			...
			//////------------------------------------------------------- Draw Earth
			glBindVertexArray(ASM_vao);
				glVertexAttrib3f(ASM_ATTRIBUTE_COLOR, 0.4f, 0.9f, 1.0f);
				glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ASM_vbo_sphere_element);
				glDrawElements(GL_TRIANGLES, ASM_gNumElements, GL_UNSIGNED_SHORT, 0);
...
void ASM_PushMatrix(mat4 data)
{

	struct ASM_stack_node* ASM_node;

	/////------------------------------ Check if Stack Depth is less than 32
	if (ASM_stackDepth > 32)
	{
		fprintf(ASM_gpFile, "Stack Overflow");
		DestroyWindow(ASM_ghwnd);
	}
	/////------------------------------ Incremnet Depth Count
	ASM_stackDepth++;

	ASM_node = (struct ASM_stack_node*)malloc(sizeof(struct ASM_stack_node));

	if (ASM_node == NULL)
	{
		fprintf(ASM_gpFile, "Unable to allocate memory to node");
		DestroyWindow(ASM_ghwnd);
	}
	...
	if (ASM_head == NULL)
	{
		...
		ASM_node->ASM_data = data;
		ASM_node->ASM_next = NULL;
		ASM_head = ASM_node;
...
mat4 ASM_PopMatrix()
{
	...
	mat4 ASM_num;
	struct ASM_stack_node* ASM_temp;
	...
	if (ASM_head == NULL)
	{
		...
		fprintf(ASM_gpFile, "Stack Underflow\n");
	...
	else
	{
		...
		ASM_num = ASM_head->ASM_data;
		ASM_temp = ASM_head;
		ASM_head = ASM_head->ASM_next;
...
void display()
{
	...
	if (ASM_temp == NULL)
	{
		...
		fprintf(ASM_gpFile, "Stack is empty\n");
	...
	else
	{
		...
		while (ASM_temp != NULL)
		{
			GLfloat* ASM_mat = (GLfloat*)ASM_temp->ASM_data;

			for (int i = 0; i < 16; i++)
			{
				fprintf(ASM_gpFile, "%f \n", *(ASM_mat + i));
			}
			
			
			ASM_temp = ASM_temp->ASM_next;
		}
