...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	ASM_hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		ASM_szAppName,
		TEXT("Akshay Circle with Line Loops OpenGL"),
		WS_OVERLAPPEDWINDOW,
...
void Circle()
{
	GLfloat ASM_angle = 0.0f;
	int ASM_i = 0;
	const int ASM_CirclePoints = 1000;
	glPointSize(4.0f);

	glBegin(GL_LINE_LOOP);
	for(ASM_i = 0; ASM_i < ASM_CirclePoints; ASM_i++)
	{
		ASM_angle = (2.0f * M_PI * ASM_i / ASM_CirclePoints);
		glColor3f(1.0f, 0.0f, 0.0f);
		glVertex3f(cos(ASM_angle), sin(ASM_angle), 0.0f);
