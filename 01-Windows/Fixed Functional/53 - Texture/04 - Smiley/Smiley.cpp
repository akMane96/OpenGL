...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	ASM_hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		ASM_szAppName,
		TEXT("Akshay Texture Simley OpenGL"),
		WS_OVERLAPPEDWINDOW,
...
void Initialize()
{
	...
	loadGLTexture(&ASM_Smiley_texture, MAKEINTRESOURCE(ASM_SMILEY_BITMAP));
...
void Cube()
{
	...
	if (ASM_PressedKey == 1)
	{
		glBindTexture(GL_TEXTURE_2D, ASM_Smiley_texture);
		glBegin(GL_QUADS);
...
void UnInitialize()
{
	...
	if (ASM_Smiley_texture)
	{
		glDeleteTextures(1, &ASM_Smiley_texture);
	}
