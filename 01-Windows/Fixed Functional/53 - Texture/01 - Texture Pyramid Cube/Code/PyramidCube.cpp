...
GLuint ASM_Stone_texture;
GLuint ASM_Kundali_texture;
...
void Initialize()
{
	...
	loadGLTexture(&ASM_Stone_texture, MAKEINTRESOURCE(ASM_STONE_BITMAP));
	loadGLTexture(&ASM_Kundali_texture, MAKEINTRESOURCE(ASM_KUNDALI_BITMAP));
...
void Cube()
{
	...
	glBindTexture(GL_TEXTURE_2D, ASM_Kundali_texture);
	//front face
	glBegin(GL_QUADS);
...
void Pyramid()
{
	...
	glBindTexture(GL_TEXTURE_2D, ASM_Stone_texture);
...
void UnInitialize()
{
	...
	if (ASM_Stone_texture)
	{
		glDeleteTextures(1, &ASM_Stone_texture);
	}

	if (ASM_Kundali_texture)
	{
		glDeleteTextures(1, &ASM_Kundali_texture);
	}
