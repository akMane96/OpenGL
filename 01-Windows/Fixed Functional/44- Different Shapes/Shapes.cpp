...
void Circle()
{
	...
	for (ASM_angle = 0.0f; ASM_angle <= (2 * M_PI); (ASM_angle = ASM_angle + 0.0001f))
	{
		...
		glVertex3f(4*sin(ASM_angle), 4*cos(ASM_angle), 0.0f);
		glEnd();
	}
...
void Graph()
{
	...
	//X Axis above
	for (ASM_i = 0; ASM_i < ASM_NoOfLines; ASM_i++)
	{
		ASM_Y = ASM_Y + ASM_gap;
		glBegin(GL_LINES);
		glColor3f(0.0f, 0.0f, 1.0f);
		glVertex3f(4.0f, ASM_Y, 0.0f);
		glVertex3f(-4.0f, ASM_Y, 0.0f);
		glEnd();
	}
