...
void Display()
{
	...
	Circle(1.0f,0.0f,0.0f, ASM_inc++);
	Circle(0.0f, 1.0f, 0.0f, ASM_inc++);
	Circle(0.0f, 0.0f, 1.0f, ASM_inc++);
	Circle(0.0f, 1.0f, 1.0f, ASM_inc++);
	Circle(1.0f, 0.0f, 1.0f, ASM_inc++);
	Circle(1.0f, 1.0f, 0.0f, ASM_inc++);
	Circle(1.0f, 1.0f, 1.0f, ASM_inc++);
	Circle(0.5f, 0.5f, 0.5f, ASM_inc++);
	Circle(1.0f, 0.5f, 0.0f, ASM_inc++);
	Circle(1.0f, 0.4f, 0.7f, ASM_inc++);
	
	SwapBuffers(ASM_ghdc);

}

void Circle(GLfloat red,GLfloat green,GLfloat blue,GLfloat ASM_inc)
{
	...
	for (ASM_angle = 0.0f; ASM_angle <= (2 * M_PI); (ASM_angle = ASM_angle + 0.0001f))
	{
		...
		glVertex3f(ASM_inc*sin(ASM_angle), ASM_inc*cos(ASM_angle), 0.0f);
		glEnd();
	}
