...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	ASM_hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		ASM_szAppName,
		TEXT("Akshay Solar System OpenGL"),
		WS_OVERLAPPEDWINDOW,
...
void Display()
{
	...
	glPushMatrix();
		...
		glRotatef(90.0f, 1.0f, 0.0f, 0.0f);					 //B
	...
	glPushMatrix();											//B
		...
		glRotatef((GLfloat)ASM_year, 0.0f, 1.0f, 0.0f);
		glTranslatef(1.0f, 0.0f, 0.0f);

		glRotatef(90.0f, 1.0f, 0.0f, 0.0f);					 //B

		glRotatef((GLfloat)ASM_day, 0.0f, 0.0f, 1.0f);  

		glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);          //B
		glColor3f(0.4f, 0.9f, 1.0f);						//B
