...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	ASM_hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		ASM_szAppName,
		TEXT("Akshay Deathly Hallows OpenGL"),
		WS_OVERLAPPEDWINDOW,
...
void DeathlyHallows()
{
	...
	GLfloat ASM_YCo[3] = { 1.0f, -1.0f, -1.0f };
	GLfloat ASM_ASide, ASM_BSide,ASM_CSide;
	GLfloat ASM_height, ASM_width;
	GLfloat ASM_XCenter, ASM_YCenter;
	GLfloat ASM_RaidusIncircle = 0.0f;
	...
	ASM_XCenter = ((ASM_BSide * ASM_XCo[0]) + (ASM_CSide * ASM_XCo[1]) + (ASM_ASide * ASM_XCo[2])) / (ASM_ASide + ASM_BSide + ASM_CSide);
	ASM_YCenter = ((ASM_BSide * ASM_YCo[0]) + (ASM_CSide * ASM_YCo[1]) + (ASM_ASide * ASM_YCo[2])) / (ASM_ASide + ASM_BSide + ASM_CSide);
