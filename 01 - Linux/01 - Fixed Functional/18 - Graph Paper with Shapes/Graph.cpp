...
void Incircle()
{
	...
	ASM_height = 4.0f / ( sqrt(2.0f));
	
	//Calculate Width of Triangle 
	ASM_width = 4.0f / (sqrt(2.0f));
	...
	for (ASM_angle = 0.0f; ASM_angle <= (2.0f * M_PI); (ASM_angle = ASM_angle + 0.0001f))
	{
		glBegin(GL_POINTS);
...
void Triangle()
{
	...
	ASM_width = 4.0f / (2 * sqrtf(2.0f));
	ASM_height = 4.0f / (2 * sqrtf(2.0f));

	glBegin(GL_LINES);
	glColor3f(1.0f, 1.0f, 0.0f);
	glVertex3f(0.0f, ASM_height, 0.0f);
	glVertex3f(-ASM_width, -ASM_height, 0.0f);
	glEnd();
...
{
	//Circle
	for (GLfloat ASM_angle = 0.0f; ASM_angle <= (2 * M_PI); ASM_angle = ASM_angle + 0.001f)
	{
		glBegin(GL_POINTS);
			glVertex3f(2*sin(ASM_angle), 2*cos(ASM_angle), 0.0f);
		glEnd();
