...
void CreateWindow()
{
	...
	XStoreName(ASM_gpDisplay, ASM_gWindow, "Checker Board");
...
void loadBitmapAsTexture()
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, ASM_CHECKIMAGEWIDTH, ASM_CHECKIMAGEHEIGHT, 0, GL_RGBA, GL_UNSIGNED_BYTE, ASM_CheckImage);
	...
	glUseProgram(ASM_shaderProgramObject);
		...
		glBindVertexArray(ASM_vao_Square);
			...
			if (ASM_PressedKey == 1)
			{
				...
				fprintf(ASM_gpFile, "ASM_PressedKey == 1\n");
			...
			} else if (ASM_PressedKey == 2)
			{
				...
				fprintf(ASM_gpFile, "ASM_PressedKey == 2\n");
