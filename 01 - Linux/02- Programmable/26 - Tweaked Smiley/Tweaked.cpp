...
GLuint ASM_choiceUniform;
GLuint ASM_textureSamplerUniform;

mat4 ASM_perspectiveProjectionMatrix;

GLuint ASM_smileyTexture;
int ASM_PressedKey = 0;
					...
					switch(ASM_keysym)
					{
						...
						case XK_F:
							if(ASM_bFullScreen == false)
							{
								...
								fprintf(ASM_gpFile, "Full Screen 1\n\n");
						...
						case XK_1:
							...
							fprintf(ASM_gpFile, "Pressed 1\n\n");
							break;
							
						case XK_2:
							ASM_PressedKey = 2;
							glEnable(GL_TEXTURE_2D);
							fprintf(ASM_gpFile, "Pressed 2\n\n");
							break;
							
						case XK_3:
							ASM_PressedKey = 3;
							glEnable(GL_TEXTURE_2D);
							fprintf(ASM_gpFile, "Pressed 3\n\n");
							break;
							
						case XK_4:
							ASM_PressedKey = 4;
							glEnable(GL_TEXTURE_2D);
							fprintf(ASM_gpFile, "Pressed 4\n\n");
							break;
							
						default:
							ASM_PressedKey = 0;
							glDisable(GL_TEXTURE_2D);
							fprintf(ASM_gpFile, "Pressed any\n\n");
							break;
					}
...
void CreateWindow()
{
	...
	XStoreName(ASM_gpDisplay, ASM_gWindow, "Tweaked Smiley");
...
void Initialize(){
	...
	ASM_choiceUniform = glGetUniformLocation(ASM_shaderProgramObject, "ASM_u_Choice");
	...
	ASM_smileyTexture = loadBitmapAsTexture("Smiley.bmp");
...
void Render(){
	...
	glUseProgram(ASM_shaderProgramObject);
		...
		glUniform1i(ASM_choiceUniform, 0);

		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, ASM_smileyTexture);
		...
		glBindVertexArray(ASM_vao_Square);
			...
			if (ASM_PressedKey == 1)
			{
				ASM_squareTexArray[0] = 1.0f;	ASM_squareTexArray[1] = 0.0f;
				ASM_squareTexArray[2] = 0.0f;	ASM_squareTexArray[3] = 0.0f;
				ASM_squareTexArray[4] = 0.0f;	ASM_squareTexArray[5] = 1.0f;
				ASM_squareTexArray[6] = 1.0f;	ASM_squareTexArray[7] = 1.0f;

			} else if (ASM_PressedKey == 2)
			{
				ASM_squareTexArray[0] = 0.5f;	ASM_squareTexArray[1] = 0.5f;
				ASM_squareTexArray[2] = 0.0f;	ASM_squareTexArray[3] = 0.5f;
				ASM_squareTexArray[4] = 0.0f;	ASM_squareTexArray[5] = 1.0f;
				ASM_squareTexArray[6] = 0.5f;	ASM_squareTexArray[7] = 1.0f;

			}
			else if (ASM_PressedKey == 3)
			{
				ASM_squareTexArray[0] = 2.0f;	ASM_squareTexArray[1] = 0.0f;
				ASM_squareTexArray[2] = 0.0f;	ASM_squareTexArray[3] = 0.0f;
				ASM_squareTexArray[4] = 0.0f;	ASM_squareTexArray[5] = 2.0f;
				ASM_squareTexArray[6] = 2.0f;	ASM_squareTexArray[7] = 2.0f;

			}
			else if (ASM_PressedKey == 4)
			{
				ASM_squareTexArray[0] = 0.5f;	ASM_squareTexArray[1] = 0.5f;
				ASM_squareTexArray[2] = 0.5f;	ASM_squareTexArray[3] = 0.5f;
				ASM_squareTexArray[4] = 0.5f;	ASM_squareTexArray[5] = 0.5f;
				ASM_squareTexArray[6] = 0.5f;	ASM_squareTexArray[7] = 0.5f;

			}
			else
			{
				glUniform1i(ASM_choiceUniform, 1);
			}
			
			glBindBuffer(GL_ARRAY_BUFFER, ASM_vbo_SquareTexture);
			glBufferData(GL_ARRAY_BUFFER, 4 * 2 * sizeof(GLfloat), ASM_squareTexArray, GL_DYNAMIC_DRAW);
...
void Uninitialize(){
	...
	if (ASM_smileyTexture)
	{
		glDeleteTextures(1, &ASM_smileyTexture);
	}
