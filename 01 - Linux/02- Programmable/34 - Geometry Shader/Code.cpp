...
GLuint ASM_geometryShaderObject;
...
void CreateWindow()
{
	...
	XStoreName(ASM_gpDisplay, ASM_gWindow, "Perspective Triangle");
...
exit(1);
	...
	//Create Shader
	ASM_geometryShaderObject = glCreateShader(GL_GEOMETRY_SHADER);

	//provide source code

	const GLchar *ASM_geometryShaderSourceCode = 
		"#version 450 core" \
		"\n" \
		"layout(triangles)in;"\
		...
		"{" \
			...
			"{"\
				...
				"gl_Position = ASM_u_mvpMatrix * (gl_in[i].gl_Position + vec4(0.0f, 1.0f, 0.0f, 0.0f));"\
				"EmitVertex();"\
				"gl_Position = ASM_u_mvpMatrix * (gl_in[i].gl_Position + vec4(-1.0f, -1.0f, 0.0f, 0.0f));"\
				"EmitVertex();"\
				"gl_Position = ASM_u_mvpMatrix * (gl_in[i].gl_Position + vec4(1.0f, -1.0f, 0.0f, 0.0f));"\
				"EmitVertex();"\
				"EndPrimitive();"\
	...
	glShaderSource(ASM_geometryShaderObject, 1, (const GLchar **)&ASM_geometryShaderSourceCode, NULL);

		//compile Shader
	glCompileShader(ASM_geometryShaderObject);

	ASM_iInfoLogLength = 0;
	ASM_iShaderCompliedStatus = 0;
	ASM_szInfoLog = NULL;

	glGetShaderiv(ASM_geometryShaderObject, GL_COMPILE_STATUS, &ASM_iShaderCompliedStatus);
	
	if (ASM_iShaderCompliedStatus == GL_FALSE)
	{
		glGetShaderiv(ASM_geometryShaderObject, GL_INFO_LOG_LENGTH, &ASM_iInfoLogLength);
		...
		if (ASM_iInfoLogLength > 0)
		{
			...
			{
				GLsizei ASM_written;
				glGetShaderInfoLog(ASM_geometryShaderObject, ASM_iInfoLogLength, &ASM_written, ASM_szInfoLog);
				fprintf(ASM_gpFile, "Geometry Shader Compilation Log : %s\n", ASM_szInfoLog);
				free(ASM_szInfoLog);
				Uninitialize();
exit(1);
			}
...
exit(1);
	...
	glAttachShader(ASM_shaderProgramObject, ASM_geometryShaderObject);
