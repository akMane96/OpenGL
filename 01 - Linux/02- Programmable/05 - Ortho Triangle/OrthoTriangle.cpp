...
mat4 ASM_orthographiccProjectionMatrix;
...
void CreateWindow()
{
	...
	XStoreName(ASM_gpDisplay, ASM_gWindow, "Ortho Triangle");
...
void Initialize(){
	...
	ASM_orthographiccProjectionMatrix = mat4::identity();
...
void Resize(int width, int height){
	...
	if (width <= height)
	{
		ASM_orthographiccProjectionMatrix = ortho(-100.0f,
			100.0f,
	...
	{
		ASM_orthographiccProjectionMatrix = ortho(-100.0f * ((GLfloat)width / (GLfloat)height),
...
void Render(){
	...
	glUseProgram(ASM_shaderProgramObject);
		...
		ASM_modelViewProjectMatrix = ASM_orthographiccProjectionMatrix * ASM_modelViewMatrix;
