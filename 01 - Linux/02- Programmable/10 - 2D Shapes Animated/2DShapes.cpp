...
GLfloat ASM_TriangleAngle;
GLfloat ASM_SquareAngle;
...
void CreateWindow()
{
	...
	XStoreName(ASM_gpDisplay, ASM_gWindow, "2D Shapes Animation");
...
void Render(){
	...
	glUseProgram(ASM_shaderProgramObject);
		...
		mat4 ASM_rotationMatrix = mat4::identity();
		...
		ASM_rotationMatrix = rotate(ASM_TriangleAngle, 0.0f, 1.0f, 0.0f);

		ASM_modelViewMatrix = ASM_translateMatrix* ASM_rotationMatrix;
		...
		ASM_rotationMatrix = mat4::identity();
		ASM_modelViewMatrix = mat4::identity();
		ASM_modelViewProjectMatrix = mat4::identity();

		ASM_translateMatrix = translate(1.5f, 0.0f, -4.0f);
		ASM_rotationMatrix = rotate(ASM_SquareAngle, 1.0f, 0.0f, 0.0f);
...
void Update()
{
	ASM_TriangleAngle = ASM_TriangleAngle + 2.0f;

	if (ASM_TriangleAngle >= 360.0f)
	{
		ASM_TriangleAngle = 0.0f;
	}

	ASM_SquareAngle = ASM_SquareAngle + 2.0f;

	if (ASM_SquareAngle >= 360.0f)
	{
		ASM_SquareAngle = 0.0f;
	}	
